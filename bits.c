...
int setFirst(int n) {
  // 3. COMPLETE
  int m = !n + ~0;
  int solution1 = ~0 << (32 + (~n + 1));
  //int solution2 = 0;
  return (solution1 & m) | 0;
}
/* 
...
int conditional(int x, int y, int z) {
  // 12. COMPLETE
  // if x then y else z
  int test = !x + ~0; // if x = 0 is 0, else is ~0
  return (test & y) | (~test & z); // if x > 0 return y, else return z
}
/* 
...
int satMul2(int x) {
  ...
  int x2 = x << 1;
  int signX = x >> 31; // all 1 or all 0
  
  int oneZeroTest = ( (x ^ x2) >> 31);
  
  return ( oneZeroTest & (signX ^ ~MIN) ) | (~oneZeroTest & x2);
}
/* 
...
int subOK(int x, int y) {
  ...
  int signXminusY = (xMinusY >> 31) & 1;
  
  int signTest = signX ^ signY; // did signs change
  int xySignTest = signXminusY ^ signX;
  
  return !(signTest & xySignTest);
}
